...
**Trigger:** Human assigns requirement IDs VC-01 → VC-07
...
| Document | Path | Why |
|---|---|---|
| Requirements | `requirements/requirements.md` | Section 4.16 (Voice Chat): VC-01–VC-07 and K-16 |
...
| `tts.ts` service with ElevenLabs + speechSynthesis | Any new canvas nodes or edges |
| `voiceChat` function in `claude.ts` | Settings UI for K-16 — that is the Settings Agent (Group 9) |
| Voice icon button in Chat panel header | ElevenLabs voice selection — post-MVP |
...
│   └── VoiceChatPanel.tsx      ✓ new component
├── lib/
│   ├── claude.ts               ✓ voiceChat function added
│   └── tts.ts                  ✓ new service
└── types/index.ts              ✓ VoiceChatMessage, VoiceChatResponse added
```
...
*Voice Agent Spec v1.0 — April 2026 (initial spec: VC-01–VC-07, K-16 dependency, Web Speech API + ElevenLabs TTS)*
