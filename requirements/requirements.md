...
| ✅ | C-18 | Each node accepts edge connections at any point along its boundary box; when the user begins dragging an edge from a node, the connection point snaps to the nearest point on the node boundary to the current cursor position; when the user drops an edge onto a target node, it attaches at the nearest boundary point to the drop position; no visible handles are shown at any time |
...
| ✅ | C-31 | Allow the user to reconnect the target end of any edge by dragging it away from its current target node and dropping it onto a different node; the edge attaches at the nearest boundary point to where it is dropped; if dropped on empty canvas or a non-node area, the edge snaps back to its original target; this applies to both single edges and to the individual branch arrows of a branching edge (the hub-to-target segments) |
...
| ✅ | A-28 | Display the Chat panel centred in the viewport at 70% of viewport width and 70% of viewport height; the panel contains a heading identifying the node being discussed, a scrollable accumulating content area where AI responses are rendered as markdown, and a bottom bar containing a text input field and a mic icon button placed immediately to the right of the input; the panel background must be rendered at 80% opacity (20% transparent), allowing the canvas behind it to show through subtly; the backdrop overlay behind the panel is unaffected |
| ✅ | A-29 | Text mode is the default on open; the user types a message in the text input and submits it (Enter key or send button); the AI responds with markdown text rendered and appended to the accumulating content area; no TTS is triggered in text mode; every AI request includes the node's label, its description (if present), and the current focus question as context |
| ✅ | A-30 | The conversation history (text and voice turns combined) persists while the Chat panel is open; history is cleared when the panel is dismissed or when Chat is opened for a different node |
...
|  | A-42 | Add a minimise button to the Chat panel header; clicking it collapses the panel to a compact horizontal bar anchored below the Menu button; the bar displays the node label, the voice state indicator (Listening / Thinking / Speaking — visible and animating only when voice mode is active), the mic toggle button, and a maximise button; no text input is shown in the minimised state |
|  | A-43 | While the Chat panel is minimised, the canvas is fully interactive — the user may pan, zoom, edit nodes, draw edges, and perform all other canvas actions; the minimised bar remains visible at all times and does not intercept canvas interaction |
|  | A-44 | Voice mode remains active when the Chat panel is minimised; the voice state indicator on the minimised bar updates in real time; clicking the mic toggle button on the minimised bar exits voice mode; TTS audio playback continues uninterrupted while minimised |
|  | A-45 | Clicking the maximise button on the minimised bar restores the Chat panel to its full size and position; all conversation history accumulated before and during the minimised state is fully preserved and visible in the restored panel |
...
| ✅ | K-13 | Add a "URL Map Generation" system prompt section to the Settings panel, positioned below the Edge Label system prompt section (K-10) and above the URL Ingestion (Jina.ai) section (K-11/K-12); same pattern as K-09 and K-10 (editable multi-line textarea, "Reset to default" button, localStorage persistence under a namespaced key, section heading "URL Map Generation"); default system prompt text: *"You are an expert knowledge assistant embedded in a concept mapping tool. Your role is to analyse the provided web page content and extract a structured concept map guided by the user's focus question. Identify the primary concepts most relevant to the focus question and the directed relationships between them, labelling each relationship with a concise phrase (1–3 words). For each primary concept, also identify its sub-concepts — more specific ideas, details, or elaborations present in the content — and describe how each sub-concept relates to its parent concept with a directional relationship label. Prioritise depth and connected structure: a well-linked map that captures both concept-to-concept and concept-to-sub-concept relationships is preferred over a flat list of loosely connected concepts."* |
| ✅ | K-15 | Add a "High contrast nodes" toggle to the Settings panel, positioned below the Theme selector (T-01); the toggle defaults to off; persist the value to `localStorage` under a namespaced key; the toggle is visible and active in both the dark and light themes; toggling the control updates the canvas immediately without requiring the Settings panel to be closed |
| ✅ | K-16 | Add an "ElevenLabs API Key" input to the Settings panel, positioned below the High contrast nodes toggle (K-15); the key is optional — Voice Chat uses browser-native `speechSynthesis` when no key is stored; if a key is stored, all Voice Chat TTS uses the ElevenLabs API with a default voice; include a "Clear" button to remove the stored key; persist under a namespaced `localStorage` key |
| ✅ | K-17 | Add an "ElevenLabs Voice ID" input to the Settings panel, positioned below the ElevenLabs API Key input (K-16); the field is optional — when empty or cleared the default voice (Rachel, `21m00Tcm4TlvDq8ikWAM`) is used; when a custom Voice ID is entered, all ElevenLabs TTS calls in Voice Chat use that voice; include a "Clear" button to revert to the default; persist under a namespaced `localStorage` key |
|  | K-18 | Add an "Enable ElevenLabs TTS" toggle to the Settings panel, positioned above the ElevenLabs API Key input (K-16); the toggle defaults to **off** on first use; persist the value to `localStorage` under a namespaced key; when the toggle is off, all Voice Chat TTS uses browser `speechSynthesis` regardless of whether a key is stored in K-16; when the toggle is on and a key is present, ElevenLabs TTS is used as normal; the toggle is visible and controllable regardless of whether a key is stored |
...
|  | V-13 | When the "High contrast nodes" setting (K-15) is enabled, render all concept nodes with (a) a subtle drop shadow and (b) a brighter node border; the exact values for each theme must be defined as named constants in `src/lib/theme.ts`; notes and branching edge hub nodes are unaffected by this setting |
...
| ✅ | U-03 | Pass the fetched page content to the Claude API with the K-13 system prompt as the `system` parameter; use the same response structure (nodes, edges, narrative, resources) and canvas rendering as Mode 1 (Generate Map); display the Summary Panel (A-16); if a focus question is set, include it as additional context alongside the fetched content |
...
| Status | ID | Requirement |
|---|---|---|
| ✅ | VC-01 | The mic icon button is positioned immediately to the right of the text input at the bottom of the Chat panel; clicking it while in text mode activates voice mode — `SpeechRecognition` starts and the state indicator changes to Listening; clicking the mic button again while in voice mode deactivates it and returns to text mode; submitting a text message while in voice mode also deactivates voice mode and returns to text mode; the mic button is dimmed and non-interactive when AI Assist is off (K-08) or when `SpeechRecognition` is unavailable in the current browser |
| ✅ | VC-02 | The Chat panel is the single unified surface for both text and voice interactions; there is no separate Voice Chat panel; the accumulating content area displays both text and voice AI responses across the session; when voice mode is active a visual state indicator (Listening / Thinking / Speaking) is displayed; a dismiss button is visible at all times |
| ✅ | VC-03 | While voice mode is active, `SpeechRecognition` runs with `continuous: true`; transcribed speech is used as the user's input turn; the turn finalises automatically on a natural pause; `SpeechRecognition` is stopped when voice mode is deactivated |
| ✅ | VC-04 | When a user speech turn is finalised, send the transcribed text to the Claude API requesting a structured response with two fields: `speech` (the conversational response to be read aloud) and `visual` (optional markdown-formatted supporting content for display in the panel); include the node label, description, focus question, and K-09 system prompt as context; use an independent conversation history separate from the Chat panel; display the Thinking state indicator while the response is pending |
| ✅ | VC-05 | When the AI response arrives, read the `speech` field aloud (ElevenLabs TTS if K-16 key stored, otherwise browser `speechSynthesis`); if the response includes a `visual` field, append its markdown content to the Voice Chat panel when audio playback begins — visual content accumulates across turns and is never replaced or cleared mid-session; if no `visual` field is present, the panel content is unchanged; while the AI is speaking, `SpeechRecognition` continues running but results are discarded to prevent TTS audio feeding back into the microphone |
| ✅ | VC-06 | If `SpeechRecognition` is unavailable in the current browser, display a clear in-panel message explaining that voice input is not supported; the panel remains openable; TTS output (AI speaking) still functions if `speechSynthesis` is available; the voice icon button in the Chat panel is dimmed and non-interactive when `SpeechRecognition` is unavailable |
| ✅ | VC-07 | The Chat panel maintains a single unified conversation history across both text and voice turns; closing the panel or opening Chat for a different node starts a fresh conversation |
| ✅ | VC-08 | When the user asks for concept suggestions during a Voice Chat session, the Claude response may include an optional `concepts` field alongside `speech` and `visual` — an array of objects each with `label` (string), `description` (string), and `relationship` (string: a concise 1–4 word edge label describing the directed relationship from the originating node to the suggested concept); the `VoiceChatResponse` type in `src/types/index.ts` must be extended to include this optional field. When a response contains a `concepts` array, render an interactive checklist block in the visual content area of the Voice Chat panel: each entry shows a checkbox, the concept label, its description, and the relationship label displayed as a small tag; an Apply button appears below the checklist, dimmed and non-interactive until at least one concept is checked; clicking Apply creates a concept node for each checked concept — populating the node's description field (C-28) — and connects each to the originating Voice Chat node using the concept's `relationship` value as the edge label; if more than one applied concept shares the same relationship label, those edges follow the branching-edge promotion pattern (A-41); differing relationship labels produce individual edges; after Apply, the checkboxes and Apply button are replaced with a confirmation indicator; the block remains in the accumulated visual content and is not cleared mid-session |
...
*Version 6.4 — April 2026 — Added K-15 (High contrast nodes toggle in Settings: dark theme only, live preview, localStorage persistence) and V-13 (concept node drop shadow and brighter border when K-15 is enabled in dark theme; theme.ts constants required; notes and hub nodes unaffected)*

*Version 6.5 — April 2026 — Updated K-15 and V-13: removed dark-theme-only restriction; high contrast nodes toggle is now active in both dark and light themes; per-theme constants required in theme.ts*

*Version 6.6 — April 2026 — Added section 4.16 Voice Chat: K-16 (ElevenLabs API key, optional, falls back to browser speechSynthesis), VC-01–VC-07 (voice icon in Chat panel header, centred visual support panel, continuous SpeechRecognition, structured speech+visual Claude response, accumulating visual content, browser support fallback, independent conversation history)*

*Version 6.7 — April 2026 — Marked K-15, K-16, VC-01–VC-07 as implemented; added K-17 (ElevenLabs Voice ID — optional custom voice, defaults to Rachel, persisted in localStorage); added VC-08 (voice-chat concept suggestions — optional concepts array in AI response, checklist UI, Apply creates nodes + edges with branching edge promotion); updated VC-05 to reflect that visual content appears when audio begins and that SpeechRecognition results are discarded during TTS playback (feedback prevention)*

*Version 6.8 — April 2026 — Updated A-28 (Chat panel redesigned as centred 70% viewport panel with accumulating content area and bottom bar with text input + mic button); updated A-29 (text mode default, no TTS, responses accumulate); retired A-31 (per-message reading view button removed); updated VC-01 (mic button next to text input toggles voice mode on/off; submitting text while in voice mode exits voice mode); updated VC-02 (Chat panel is the single unified surface for text and voice — no separate Voice Chat panel); updated VC-03 (SpeechRecognition active only while voice mode is on, stopped on deactivation); updated VC-07 (single unified conversation history across text and voice turns)*

*Version 6.9 — April 2026 — Added K-18 (Enable ElevenLabs TTS toggle: positioned above K-16 API key input, defaults off, persisted to localStorage; when off, all Voice Chat TTS uses browser speechSynthesis regardless of stored key)*

*Version 7.0 — April 2026 — Updated A-28: Chat panel background rendered at 80% opacity (20% transparent); backdrop overlay unaffected*

*Version 7.1 — April 2026 — Added A-42 (minimise button collapses Chat panel to horizontal bar below Menu button showing node label, voice indicator, mic toggle, maximise button), A-43 (canvas fully interactive while minimised; bar does not intercept canvas), A-44 (voice mode remains active while minimised; mic toggle exits voice mode; TTS continues), A-45 (maximise restores full panel with full history)*
