...
|  | VC-05 | When the AI response arrives, read the `speech` field aloud (ElevenLabs TTS if K-16 key stored, otherwise browser `speechSynthesis`); if the response includes a `visual` field, append its markdown content to any previously displayed content in the Voice Chat panel — visual content accumulates across turns and is never replaced or cleared mid-session; if no `visual` field is present, the panel content is unchanged; while the AI is speaking keep `SpeechRecognition` running; if the user begins speaking, stop TTS playback immediately and process the new input as the next turn |
