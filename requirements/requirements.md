...
| Status | ID | Requirement |
|---|---|---|
| ✅ | VC-01 | Add a voice icon button to the Chat panel header (A-28); clicking it dismisses the Chat panel and opens the Voice Chat panel in its place; if the Summary Panel is visible it is also dismissed; only one of Chat panel, Voice Chat panel, or Summary Panel is visible at a time; the button is dimmed and non-interactive when AI Assist is off (K-08) or when `SpeechRecognition` is unavailable in the current browser |
| ✅ | VC-02 | Display the Voice Chat panel centred in the viewport at the same dimensions as the Reading panel (A-32: 70% viewport width and height); the panel serves as a visual support surface where the AI may render supplementary content (key terms, structured lists, resource links, or other supporting material) when relevant to the spoken response; the panel also displays a visual state indicator (Listening / Thinking / Speaking) and a dismiss button; when no supporting content is available the content area is empty |
| ✅ | VC-03 | While the Voice Chat panel is open, the microphone is always active using the browser Web Speech API (`SpeechRecognition` with `continuous: true`); transcribed speech appears internally as the user's input; the turn is finalised automatically when the user stops speaking (natural pause detection by the browser) |
...
| ✅ | VC-07 | The Voice Chat panel maintains an independent conversation history from the Chat panel; closing and reopening Voice Chat for the same node starts a fresh conversation |
