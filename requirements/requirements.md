...
|  | V-13 | When the "High contrast nodes" setting (K-15) is enabled and the dark theme is active, render all concept nodes with (a) a subtle drop shadow and (b) a brighter node border; both values must be defined as named constants in `src/lib/theme.ts`; notes and branching edge hub nodes are unaffected by this setting |
